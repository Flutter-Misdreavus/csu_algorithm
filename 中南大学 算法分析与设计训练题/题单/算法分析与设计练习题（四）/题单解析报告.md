...
- **对应章节**：第 8 章动态规划
- **题目总数**：21 道
- **核心算法思想**：动态规划各类模型（线性 DP、树形 DP、区间 DP、背包 DP、计数 DP、记忆化搜索）、图论（Floyd、传递闭包、离线最短路）、组合计数与容斥

### 1.2 知识点总览

| 题号 | 题目名称 | 核心知识点 | 难度 |
|------|----------|-----------|------|
| B3611 | 【模板】传递闭包 | 图论、传递闭包、Floyd | 简单 |
| B3647 | 【模板】Floyd | 图论、最短路、Floyd-Warshall | 简单 |
...
- **记忆化搜索**：Function（P1464）、滑雪（P1434）
- **图论与 Floyd**：传递闭包（B3611）、全源最短路（B3647）、离线增量最短路（P1119）
- **最短路 DP**：租用游艇（P1359）
- **计数/时间轴 DP**：摆渡车（P5017）
- **组合计数与容斥**：Emiya 家今天的饭（P5664）
...
$$b[i][j] = b[i][j] \lor (b[i][k] \land b[k][j])$$
...
时间复杂度：三重循环，$O(n^3)$。当 $n = 100$ 时完全可行。
空间复杂度：邻接矩阵 $O(n^2)$。
...
>
> 数据范围：$n \le 100$，$m \le 4500$，$1 \le w \le 1000$。
...
使用二维数组 `dis[i][j]` 记录从 $i$ 到 $j$ 的最短距离。算法选择 **Floyd-Warshall**，它适合求解所有点对最短路，时间复杂度为 $O(n^3)$。由于 $n \le 100$，完全在可接受范围内。
...
时间复杂度：三重循环，$O(n^3)$。当 $n = 100$ 时约为 $10^6$ 次运算。
空间复杂度：二维距离矩阵，$O(n^2)$。
...
- 对每个询问 $(x, y, t)$：
  - 将所有 $t[i] \le t$ 且尚未加入的村庄 $i$ 依次作为中间点，执行 Floyd 松弛。
...
时间复杂度：最坏情况下所有 $N$ 个村庄都会被加入，每次加入执行 $O(N^2)$ 松弛，加上 $Q$ 次查询，总复杂度为 $O(N^3 + Q)$。
空间复杂度：距离矩阵 $O(N^2)$。
...
> 长江上有 $n$ 个游艇出租站 $1, 2, \dots, n$。游客可在出租站租用游艇，并在下游任意出租站归还。已知从出租站 $i$ 到出租站 $j$（$i \lt j$）的租金 $r(i,j)$。求从出租站 $1$ 到出租站 $n$ 所需的最少租金。
>
> 数据范围：$1 \le n \le 200$，保证计算过程中任何时刻数值不超过 $10^6$。
...
由于站点只能按编号递增方向前进，从 $1$ 到 $n$ 的最低租金问题满足最优子结构：如果从 $1$ 到 $j$ 的最优路径中，最后一步是从某个 $i$（$i \lt j$）直接到达 $j$ 的，那么从 $1$ 到 $i$ 也必须是最优的。因此可以按站点编号从小到大递推。

**3. 数据结构与算法选择**

使用一维数组 `dp[j]` 表示从出租站 $1$ 到出租站 $j$ 的最少租金。状态转移方程：

$$dp[j] = \min_{1 \le i \lt j}(dp[i] + r[i][j])$$
...
时间复杂度：枚举每个 $j$ 及其所有前驱 $i$，共 $O(n^2)$。
空间复杂度：需要存储租金矩阵和 dp 数组，$O(n^2)$。
...
> 给定一个 $R \times C$ 的二维高度矩阵，每个格子有一个高度值。一个人可以从某个点滑向上下左右相邻四个点之一，当且仅当目标点的高度严格小于当前点。求区域中最长滑坡的长度。
>
> 数据范围：$1 \le R, C \le 100$，$0 \le h \le 10000$。
...
对于任意一个格子 $(x, y)$，从它出发的最长滑坡只依赖于它四周比它低的那些格子。如果我们已经知道了所有更低邻居的最长滑坡长度，那么从 $(x, y)$ 出发的最长长度就是：

$$f[x][y] = \max_{(nx,ny) \text{ 更低}} f[nx][ny] + 1$$

这种“大问题分解为已解决的子问题”的结构，正是动态规划的核心。
...
空间复杂度：需要两个 $R \times C$ 的数组以及递归栈，均为 $O(RC)$。
...
>
> 数据范围：$n \le 500$，$m \le 100$，$0 \le t[i] \le 4\times 10^6$。
...
最优策略中，每趟车的发车时刻一定等于某名同学的到达时刻或边界情况。

**3. 数据结构与算法选择**

设 `f[i]` 表示将时间轴上 $(-\infty, i]$ 分段，且最后一段右边界为 $i$ 时，所有同学等待时间之和的最小值。

设 `cnt[i]` 为到达时刻 $\le i$ 的同学人数，`sum[i]` 为这些人的到达时刻之和，通过前缀和预处理。

状态转移：枚举上一段右边界 $j$，满足 $j \le i - m$（保证段长至少 $m$），则：

$$f[i] = \min_{j \le i-m}\big\{ f[j] + (cnt[i] - cnt[j]) \times i - (sum[i] - sum[j]) \big\}$$

其中 $(cnt[i] - cnt[j]) \times i - (sum[i] - sum[j])$ 表示在 $(j, i]$ 这段时间内到达的同学，每人等待 $i - t[k]$ 之和。

边界情况：从时间 $0$ 开始作为第一段，$f[i] = cnt[i] \times i - sum[i]$。
...
- 对 `f[i]` 进行递推：
  - 若 $i \ge m$ 且 $cnt[i - m] == cnt[i]$，说明 $(i - m, i]$ 内无人到达，这段可以省略，令 `f[i] = f[i - m]`。
  - 否则先令 `f[i] = cnt[i] * i - sum[i]`。
  - 再枚举 $j$ 从 $\max(i - 2m + 1, 0)$ 到 $i - m$ 更新 `f[i]`。这里利用了“段长 $\ge 2m$ 时总可以再切一刀，不会更劣”的剪枝性质。
- 答案在 $i \in [t_{\max}, t_{\max} + m)$ 范围内取最小值。

**5. 时空复杂度分析**

时间复杂度：通过剪枝，有效位置不超过 $nm$ 个，每个位置枚举 $O(m)$ 个转移，总复杂度 $O(nm^2 + t_{\max})$，可以接受。
空间复杂度：需要 `cnt`、`sum`、`f` 三个长度为 $t_{\max} + m$ 的数组，$O(t_{\max})$。
...
### 2.21 P5664 [CSP-S 2019] Emiya 家今天的饭
...
>
> 数据范围：$n \le 100$，$m \le 2000$，$0 \le a[i][j] \lt 998244353$。
...
$$\text{total} = \prod_{i=1}^{n}\left(1 + \sum_{j=1}^{m} a[i][j]\right) - 1$$

减 $1$ 是排除全不选的情况。

现在考虑食材限制：设最终选了 $k$ 道菜，食材 $j$ 出现 $x_j$ 次，则要求 $x_j \le \lfloor k/2 \rfloor$。

关键观察：**不可能有两种食材同时不合法**。若食材 $j_1$ 和 $j_2$ 都出现超过 $k/2$ 次，则总菜数 $x_{j1} + x_{j2} > k$，矛盾。因此“存在不合法食材”这一事件中的各食材方案集合互不相交，可以直接用总方案数减去每个食材的不合法方案数，无需复杂容斥。

**3. 数据结构与算法选择**

对每个食材 $j$，计算“食材 $j$ 出现次数超过一半”的方案数。将每个方法 $i$ 的选择分为三类：不选、选食材 $j$、选非 $j$ 食材。设选食材 $j$ 的方案数为 $a[i][j]$，选非 $j$ 食材的方案数为 $c[i] = \sum_{l\ne j} a[i][l]$。

由于要求食材 $j$ 出现次数 $x >$ 总菜数 $k/2$，即 $x > y$（$y$ 为非 $j$ 食材出现次数）。令差值 $d = x - y$，则只需 $d \ge 1$。
...
- 对每个食材 $j$：
  ...
  - 不合法方案数 $\text{bad}[j] = \sum_{d > n} dp[d]$。
- 答案 $\text{ans} = \text{total} - \sum \text{bad}[j]$，取模修正。

**5. 时空复杂度分析**

时间复杂度：对每个食材 $j$ 做 $O(n^2)$ 的 DP，共 $O(m \cdot n^2) = 2000 \times 100^2 = 2\times 10^7$。
空间复杂度：$O(n^2 + n \cdot m)$。
...
- **P1434 [SHOI2002] 滑雪**：网格中状态依赖关系由高度决定，不是简单线性递推，用记忆化搜索自然处理“从高处滑向低处”的 DAG 结构。

**7. 图论中的 Floyd 与传递闭包**

Floyd 算法既可以求所有点对最短路，也可以做布尔传递闭包，本质都是“通过中间点扩展已知关系”。

- **B3611 传递闭包**：用布尔版 Floyd 计算有向图中任意两点是否可达。
- **B3647 Floyd 模板**：计算无向带权图的全源最短路。
- **P1119 灾后重建**：利用 Floyd 增量特性，按时间离线加入新开放的村庄作为中间点。

**8. 最短路 DP**

- **P1359 租用游艇**：站点只能向下游移动，形成天然拓扑序，用一维 DP 按编号递增递推即可。

**9. 计数 DP 与组合数学**

- **P5017 摆渡车**：在时间轴上做 DP，用前缀和快速计算某段时间内到达同学的等待时间之和，配合剪枝优化。
- **P5664 Emiya 家今天的饭**：先用乘法原理计算无限制总方案，再用 DP 对每个食材统计“出现次数超过一半”的不合法方案数，利用“最多一个食材不合法”的性质做减法容斥。
...
| 网格记忆化 | P1434 | `f[x][y]` 从低邻居转移 | 记忆化 DFS |
| 图论（Floyd） | B3611, B3647, P1119 | 距离/可达矩阵 | 中间点 k 松弛 |
| 最短路 DP | P1359 | `dp[j]` 到第 j 站最小租金 | 拓扑序递推 |
| 计数/时间轴 DP | P5017 | `f[i]` 最后一段右端点为 i | 前缀和 + 剪枝 |
| 组合计数 | P5664 | `dp[d]` 食材出现差值 | 总方案减不合法方案 |
...
8. **Floyd 的多功能性**：Floyd 不仅可以求最短路，还可以通过布尔运算求传递闭包；按时间增量加入中间点可处理离线动态图问题（B3611、B3647、P1119）。
9. **利用拓扑序递推**：当问题具有天然的顺序约束时（如 P1359 中只能向下游行驶），可以把最短路问题转化为一维线性 DP。
10. **前缀和优化 DP 转移**：当 DP 转移需要快速计算某段区间内元素之和时，预处理前缀和可以将每次转移从 $O(n)$ 降到 $O(1)$（P5017）。
11. **记忆化搜索处理 DAG**：当状态依赖关系构成 DAG 但不易按顺序枚举时，用递归加记忆化搜索自然处理（P1434）。
12. **组合计数中的减法容斥**：当直接统计合法方案困难时，可以先统计无限制总方案，再减去不合法方案；若“多个不合法事件互斥”，则无需复杂容斥（P5664）。
