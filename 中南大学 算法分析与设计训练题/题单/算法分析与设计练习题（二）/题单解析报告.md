...
- **对应章节**：第 4 章减治法 + 第 5 章分治法
- **题目总数**：7 道
- **核心算法思想**：分治、递归、归并排序、DFS 枚举、快速选择
...
本题单围绕**分治法**和**减治法**展开：
- **分治框架**：归并排序（P1908、P12890）、最近点对（P1257/P1429）、递归分形（P1498、P5461）
...
2. **分治的合并优化**：P1429 展示了如何通过 `merge` 维护 y 序，将 $O(n\log^2 n)$ 优化到 $O(n\log n)$。
